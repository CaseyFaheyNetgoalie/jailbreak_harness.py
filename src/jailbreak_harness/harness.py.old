...
def load_test_suite_from_file(filepath: str) -> List[TestCase]:
    ...
    try:
        ...
        if not isinstance(test_list, list):
            raise ValueError(
                "Test suite file must contain a top-level list of test cases or a 'tests' key containing a list."
            )
        ...
        logger.info(
            f"Successfully loaded {len(test_cases)} total test cases from {filepath}."
        )
        return test_cases
...
class JailbreakHarness:
    ...
    def __init__(self, model_caller: BaseModelCaller, model_name: str = "mock-model"):
        ...
        self._last_export_ts: Optional[
            str
        ] = None  # Stores timestamp for consistent exports
    ...
    def run_variant(
        ...
        try:
            ...
            logger.exception(
                f"Model caller {type(self.model_caller).__name__} failed internally"
            )
            resp = self.model_caller.format_error_response(
                f"CallerInternalError_{type(e).__name__}"
            )
    ...
    def run_tests(
        ...
        for seed in range(seeds):
            ...
            for t, variant in tqdm(
                variant_runs, desc=f"Running variants (seed {seed})", unit="test"
            ):
                run_id += 1
    ...
    def summary(self):
        ...
        for tid, info in by_test.items():
            logger.info(
                f" {tid} ({info['runs']} runs): Unique responses={len(info['unique_responses'])}, Safety flags={info['safety_flags']}, Errors={info['errors']}"
            )
...
def main():
    ...
    try:
        ...
        logger.error(
            f"Invalid temperature value found in '{args.temperatures}'. Must be a comma-separated list of numbers."
        )
        return
